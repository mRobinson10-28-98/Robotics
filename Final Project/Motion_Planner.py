...
class Motion_Planner(Quat_Math):
    ...
    def iteration(self):
        ...
        self.config_prime = self.config + self.beta * np.matmul(self.B, (self.gamma_prime - self.gamma))
    ...
    def spatial_manip_jac(self, thetas):
        ...
        for joint in range(n):
            ...
            g1_is.append(np.matmul(g1_is[joint], transformation))
        ...
        for element in range(len(g1_is)):
            ...
            j_column = np.matmul(adjoint, joint_twists[element])
    ...
    def transformation(self, axis_joint, q_joint, theta):
        ...
        rot = I + skew * m.sin(t) + np.matmul(skew, skew) * (1 - m.cos(t))
        a = (I - rot)
        trans = np.matmul(a, q_joint)
    ...
    def B_matrix(self):
        ...
        top_right_3by3 = 2 * np.matmul(p_hat, J1)
        J2 = np.vstack((np.hstack((np.identity(3), top_right_3by3)), np.hstack((np.zeros((3,3)), 2 * J1))))
        B = np.matmul(np.matmul(self.spatial_jac.transpose(), np.linalg.inv(np.matmul(self.spatial_jac, self.spatial_jac.transpose()))), J2)
        return B
    ...
    def manipdkin(self, thetas):
        ...
        for index in range(len(thetas)):
            ...
            if self.type_joints[index] == "R":
                rot = I + skew_axis[index] * m.sin(t) + np.matmul(skew_axis[index], skew_axis[index]) * (1 - m.cos(t))
                a = (I - rot)
                trans = np.matmul(a, q)
        ...
        for transformation_matrix in transformation_matrices:
            g_theta = np.matmul(g_theta, transformation_matrix)
